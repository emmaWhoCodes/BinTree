...
/*------------------------------- buildTree ----------------------------------
 * Pre: Takes in a reference to a BinTree, T, which will be the BinTree that
 *      the data is added to. Takes in a reference to an ifstream, infile,
 *      that reads the data from the desired file.
 *
 * Post: Read strings from a line, terminating when "$$" is encountered.
 *       Only adds values to the tree that have unique values. Does not add
 *       values that are not unique, and deletes them instead. Since there is
 *       some work to do before the actual insert that is specific to the
 *       client problem, it's best that building a tree is not a  member
 *       function.  It's a global function.
 *---------------------------------------------------------------------------*/
...
/*------------------------------- initArray ----------------------------------
 * Pre: Takes in an array, ndArray, that stores NodeData pointers as its data.
 *
 * Post: Initializes the array of NodeData* to NULL pointers
 *---------------------------------------------------------------------------*/
